...
image_version="2.6.6"
haproxy_admin_port=1080
frontend_es_port=19200
frontend_fsir_port=80
...
setup_docker_deployment_configs() {
  ...
  container_volume_mappings=(
    "${container_local_conf_dir}/haproxy.cfg:/usr/local/etc/haproxy/haproxy.cfg",
  )
...
step_1_prepare_before_install() {
  ...
  check_if_file_exists "${container_local_conf_dir}/haproxy.cfg"
}
